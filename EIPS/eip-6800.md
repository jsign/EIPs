...
```
# Pedersen basis of length 256
PEDERSEN_BASIS = [....]
VERKLE_NODE_WIDTH = len(PEDERSEN_BASIS)

def group_to_field(element: GroupElement) -> int:
    # Not collision resistant. Not random oracle. 
    # Binding for Pedersen commitments.
    assert isinstance(element, GroupElement)
    if element == group.Z:
        return 0
    else:
        return element.map_to_scalar_field()
    
def compute_commitment_root(children: Sequence[int]) -> GroupElement:
    o = group.Z
    for generator, child in zip(PEDERSEN_BASIS, children):
        o = group.add(o, group.scalar_mul(generator, child))
    return o
...
def compute_verkle_root(data: Dict[bytes32, bytes32]) -> GroupElement:
...
The concrete group to be used is the Bandersnatch prime-order subgroup.
...
def pedersen_hash(inp: bytes) -> bytes32:
    ...
    return compute_commitment_root(ints).hash()
